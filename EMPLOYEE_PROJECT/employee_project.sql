...
SELECT p.PNAME AS Project_Name, COUNT(a.EMPNO) AS Headcount
FROM PROJECT p
...
SELECT e.EMPNO, e.ENAME, p.PNAME AS Project_Name
FROM EMPLOYEE e
JOIN ASSIGNED_TO a ON e.EMPNO = a.EMPNO
JOIN PROJECT p ON a.PNO = p.PNO
WHERE p.PNAME = 'Alpha';

SELECT d.DNAME AS Department_Name, d.DLOC AS Location
FROM DEPT d
